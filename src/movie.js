const $ = require('jquery');
const yellowStarUrl = "data:image/svg+xml;base64,";
const darkStarUrl = "data:image/svg+xml;base64,";
const totalStars = 5;

const getRatingElement = (rating) => {
    let $ol = $("<ol></ol>").addClass("starRatings");
    let $li = $("<li class='starRating'></li>");
    $li.attr('data-rating', rating);
    let remainingStars = rating;
    for (let i = 0; i < totalStars; i++) {
        let star = $("<img>");
        if (remainingStars > 0) {            
            star.attr('src', yellowStarUrl);            
            remainingStars--;
        }
        else {
           star.attr('src', darkStarUrl);
        }
        $li.append(star);
    }
    $ol.append($li);
    return $ol;
}

class Movie {
    constructor(title, posterUrl, rating) {
        this.title = title;
        this.rating = Math.floor(rating);
        this.posterUrl = posterUrl;
    }

    getElement() {
        let movieContainer = $("<div></div>").addClass("col-3 movie");
        let titleEl = $("<h4></h4>").text(this.title);
        let posterEl = $("<img>");
        posterEl.attr('src', this.posterUrl);
        let ratingEl = getRatingElement(this.rating);
        movieContainer.append(titleEl);
        movieContainer.append(posterEl);
        movieContainer.append(ratingEl);

        return movieContainer;
    }
}



export default Movie;